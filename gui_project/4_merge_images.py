...
root = Tk()
root.title("Samuel Photo")

# 파일 추가
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
                                        filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), \
                                        initialdir=r"C:\Users\souls\PycharmProjects\Tkinter-project1\gui_project")
    # 사용자가 선택한 파일 목록 인쇄
    for file in files:
...
def merge_image():
    ...
    dest_path = os.path.join(txt_dest_path.get(), "samuel_photo.jpg")
...
root.resizable(False, False) # 너비, 높이 값 변경 블가
root.mainloop()
